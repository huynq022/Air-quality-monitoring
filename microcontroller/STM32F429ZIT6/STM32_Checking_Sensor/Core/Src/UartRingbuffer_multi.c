...
void Uart_flush (UART_HandleTypeDef *uart)
{
	...
	if (uart == pc_uart)
	{
		_rx_buffer3->head = _rx_buffer3->tail;
	}
}
...
{
	if (uart == device_uart) return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer2->head - _rx_buffer2->tail) % UART_BUFFER_SIZE;
	else if (uart == pc_uart) return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer3->head - _rx_buffer3->tail) % UART_BUFFER_SIZE;
	return -1;
}
...
again:
	...
	if (uart == device_uart)
		...
		while (Uart_peek(uart) != string[so_far]) _rx_buffer3->tail = (unsigned int)(_rx_buffer3->tail + 1) % UART_BUFFER_SIZE;
	while (Uart_peek(uart) == string [so_far])
	{
		so_far++;
		Uart_read(uart);
...
int16_t Get_position (char *string, UART_HandleTypeDef *uart)
{
  ...
  else if (uart == pc_uart)
  {
	 ...
	 unsigned int start = _rx_buffer3->tail;
	 unsigned int end = _rx_buffer3->head;
	 for (unsigned int i=start; i<end; i++)
	 {
	  if (Uart_read(pc_uart) == string[so_far])
		{
