...
const COChart = () => {
  ...
  const realChart = {
    ...
    title: {
      text: `Historical data of CO value on ${formatDate(currentDate)}`,
    },
    subtitle: {
  ...
  const predictChart = {
    ...
    title: {
      text: `Forecasted data of CO value for next hour on ${formatDate(
        currentDate
      )}`,
    },
    subtitle: {
  ...
  const selectOption = async (option) => {
    switch (option.value) {
      case "Prophet":
        setLoading(true);
        try {
          await predictCOWithProphet(chartData.obj).then((result) => {
      ...
      case "LSTM":
        setLoading(true);
        try {
          await predictCOWithLSTM(dataCO).then((result) => {
            setCheckPredict(true);
            setPredictData({
              timeData: chartData.timeDataPredict,
              seriesData: result.data.forecast,
  ...
  useEffect(() => {
    fetchDataCOHCM().then(async (result) => {
      ...
      result.data.feeds.forEach((entry) => {
        ...
        objFormat.value.push(entry.field4);
      });
    ...
    setInterval(fetchDataCOHCM, 5 * 60 * 1000);
  }, []);

  useEffect(() => {
    get100DataOfCOHCM().then((result) => {
      let objFormat = {
        time: [],
        value: [],
      };
